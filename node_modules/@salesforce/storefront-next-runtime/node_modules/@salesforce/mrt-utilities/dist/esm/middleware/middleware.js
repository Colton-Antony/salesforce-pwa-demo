 ...
 * ```
 */
export const createMRTProxyMiddlewares = (proxyConfigs, appProtocol = 'http', includeCaching = false, createProxyFn, accessControlHeaderForwardingHostnames = DEFAULT_ACCESS_CONTROL_FORWARDING_HOSTNAMES, preserveUserAgent = true) => {
    if (!proxyConfigs) {
        return [];
    }
    const { appHostname } = getRequestProcessorParameters();
    const proxies = configureProxying(proxyConfigs, appHostname, appProtocol, createProxyFn, accessControlHeaderForwardingHostnames, preserveUserAgent);
    const middlewares = [];
