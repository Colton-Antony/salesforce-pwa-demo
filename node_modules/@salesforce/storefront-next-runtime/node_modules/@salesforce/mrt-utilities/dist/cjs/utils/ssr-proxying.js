...
exports.rewriteProxyRequestHeaders = exports.ALLOWED_CACHING_PROXY_REQUEST_HEADERS = exports.X_HEADERS_TO_ADD = exports.X_HEADERS_TO_REMOVE_ORIGIN = exports.hostnameMatchesTransformationList = exports.DEFAULT_ACCESS_CONTROL_FORWARDING_HOSTNAMES = exports.X_HEADERS_TO_REMOVE_PROXY = exports.rewriteProxyResponseHeaders = exports.rewriteSetCookies = exports.rewriteDomain = exports.parseHost = exports.cookieAsString = exports.rfc1123 = exports.Headers = exports.MAX_URL_LENGTH_BYTES = exports.X_MOBIFY_REQUEST_CLASS = exports.X_PROXY_REQUEST_URL = void 0;
/**
...
exports.X_PROXY_REQUEST_URL = 'x-proxy-request-url';
exports.X_MOBIFY_REQUEST_CLASS = 'x-mobify-request-class';
exports.MAX_URL_LENGTH_BYTES = 8192;
/**
...
class Headers {
    ...
    toAWSFormat() {
        ...
        for (const [key, values] of Object.entries(this.headers)) {
            ...
            const finalKey = key.includes('_') ? key : (0, change_case_1.trainCase)(key);
...
const cookieAsString = (cookie) => {
    ...
    if (cookie.expires) {
        ...
        elements.push(`Expires=${(0, exports.rfc1123)(cookie.expires)}`);
    }
    if (cookie.domain) {
...
const rewriteDomain = (domain, appHostname, targetHost) => {
    ...
    // RFC3986
    const parsedHost = (0, exports.parseHost)(appHostname);
 ...
 */
const rewriteSetCookies = ({ appHostname, setCookies, targetHost, logging = false, }) => {
    if (!(setCookies && setCookies.length)) {
        return [];
    }
    // Parse the set-cookie headers into a set of objects
    const oldCookies = (0, set_cookie_parser_1.parse)(setCookies, { decodeValues: false });
    // Map the oldCookies array into an array of updated objects
    const newCookies = oldCookies.map((cookie) => {
        if (cookie.domain) {
            const newDomain = (0, exports.rewriteDomain)(cookie.domain, appHostname, targetHost);
    ...
    return newCookies.map(exports.cookieAsString);
};
 ...
 */
const rewriteProxyResponseHeaders = ({ appHostname, caching, headers, headerFormat = 'http', proxyPath, requestUrl, statusCode = 200, targetProtocol, targetHost, appProtocol = 'https', logging = false, }) => {
    ...
    if (requestUrl) {
        ...
        // value to 8192 bytes.
        const fullRequestUrl = (requestUrl.startsWith('/') ? `${targetOrigin}${requestUrl}` : requestUrl).slice(0, exports.MAX_URL_LENGTH_BYTES);
        if (logging) {
            console.log(`Setting proxy response ${exports.X_PROXY_REQUEST_URL} header to "${fullRequestUrl}"`);
        }
        workingHeaders.setHeader(exports.X_PROXY_REQUEST_URL, fullRequestUrl);
    }
    ...
    if (caching) {
        ...
        const updatedCookies = (0, exports.rewriteSetCookies)({
            appHostname,
    ...
    if (statusCode >= 301 && statusCode <= 308) {
        ...
        if (location) {
            ...
            const locUrl = new url_1.URL(location, targetOrigin);
 ...
 * @private
 */
exports.X_HEADERS_TO_REMOVE_PROXY = ['x-mobify-access-key'];
const X_SFDC_ACCESS_CONTROL = 'x-sfdc-access-control';
exports.DEFAULT_ACCESS_CONTROL_FORWARDING_HOSTNAMES = ['.commercecloud.salesforce.com'];
const hostnameMatchesTransformationList = (hostname, hostnameSuffixes) => {
...
exports.hostnameMatchesTransformationList = hostnameMatchesTransformationList;
/**
 * List of x- headers that are removed from origin requests.
 * @private
 */
exports.X_HEADERS_TO_REMOVE_ORIGIN = [
    'x-api-key',
    'x-apigateway-event',
 ...
 * @private
 */
exports.ALLOWED_CACHING_PROXY_REQUEST_HEADERS = {
 ...
 */
const rewriteProxyRequestHeaders = ({ caching = false, headers, headerFormat = 'http', targetProtocol, targetHost, logging = false, accessControlHeaderForwardingHostnames, preserveUserAgent = true, }) => {
    if (!headers) {
        return {};
    }
    const workingHeaders = new Headers({ ...headers }, headerFormat);
    // Strip out some specific X-headers
    exports.X_HEADERS_TO_REMOVE_PROXY.forEach((key) => workingHeaders.deleteHeader(key));
    // Conditionally strip x-sfdc-access-control.
    // Forward it only for non-caching requests to hosts matching the suffix list.
    if (caching || !(0, exports.hostnameMatchesTransformationList)(targetHost, accessControlHeaderForwardingHostnames)) {
    ...
    if (caching) {
        ...
        workingHeaders.keys().forEach((key) => {
            if (!exports.ALLOWED_CACHING_PROXY_REQUEST_HEADERS[key]) {
    ...
    Object.entries(exports.X_HEADERS_TO_ADD).forEach(([key, value]) => {
