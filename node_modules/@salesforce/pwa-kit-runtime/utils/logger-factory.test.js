...
describe('PWAKitLogger', () => {
  ...
  _log('--- NOTE: in this file, `console.log` is mocked. So use `_log` if you need access to the original method.');
  beforeEach(() => {
  ...
  describe('serializeError method', () => {
    let logger;
    beforeEach(() => {
      logger = (0, _loggerFactory.default)({
        packageName: 'test-package'
      });
    });
    test('should serialize Error objects with name, message, and stack', () => {
      ...
      expect(console.info).toHaveBeenCalledWith(`test-package INFO Error occurred {"error":${JSON.stringify(expectedErrorObj)}}`);
    });
    test('should serialize multiple Error objects in the same object', () => {
      ...
      expect(console.info).toHaveBeenCalledWith(`test-package INFO Multiple errors occurred ${JSON.stringify(expectedObject)}`);
    });
    test('should handle custom Error types', () => {
      ...
      expect(console.info).toHaveBeenCalledWith(`test-package INFO Custom error test {"error":${JSON.stringify(expectedError)}}`);
    });
    test('should leave non-Error properties unchanged', () => {
      ...
      expect(console.info).toHaveBeenCalledWith(`test-package INFO Object with error and other properties ${JSON.stringify(expectedObject)}`);
    });
    test('should handle objects with no Error properties', () => {
      ...
      expect(console.info).toHaveBeenCalledWith(`test-package INFO Plain object test ${JSON.stringify(plainObject)}`);
    });
    ...
    test('should handle Error objects without stack property', () => {
      ...
      expect(console.info).toHaveBeenCalledWith(`test-package INFO Error without stack {"error":${JSON.stringify(expectedError)}}`);
    });
  });
