...
var _opentelemetry = require("./opentelemetry");
...
class PerformanceTimer {
  ...
  mark(name, type, options = {}) {
    ...
    if (type !== this.MARKER_TYPES.START && type !== this.MARKER_TYPES.END) {
      _loggerInstance.default.warn('Invalid mark type', {
        type,
        name,
    ...
    try {
      ...
      if (type === this.MARKER_TYPES.START) {
        if (!this.spans.has(name)) {
          const span = (0, _opentelemetry.createChildSpan)(name, {
          ...
          _loggerInstance.default.warn('Span already exists', {
            name,
            namespace: 'PerformanceTimer.mark'
          });
        }
        ...
        try {
          ...
          _loggerInstance.default.error('Failed to measure performance mark', {
            name,
            error: error.message,
            startMark,
            endMark,
      ...
      if (error.name === 'SyntaxError') {
        _loggerInstance.default.error('Invalid performance mark name', {
          name,
          error: error.message
        });
      } else {
        _loggerInstance.default.error('Error creating performance mark:', {
          name,
          type,
          error: error.message,
          namespace: 'PerformanceTimer.mark'
  ...
  _cleanupOrphanedSpan(name, reason = 'manual') {
    const span = this.spans.get(name);
    if (span) {
      _loggerInstance.default.warn('Cleaning up orphaned span', {
        name,
