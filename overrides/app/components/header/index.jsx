            ...
            <Box bg="#f8f9fa" borderTop="1px solid" borderColor="gray.200" px={4}>
                <Flex maxW="container.xl" mx="auto" gap={8} fontSize="sm" fontWeight="semibold" color="gray.700">
                    {navItems && navItems.map((tab) => (
                        <Box key={tab._key} position="relative" role="group">
                                ...
                                >
                                    ...
                                    {tab.megaMenuCategories && tab.megaMenuCategories.map(cat => (
                                        <Box 
                                            ...
                                            h="full"
                                            bg="white"
                                            boxShadow="6px 0 10px -4px rgba(0,0,0,0.1)"
                                            zIndex={20}
                                            py={2}
                                            borderRight="1px solid" 
                                            borderColor="gray.200"
                                        >
                                            {cat.subLinks && cat.subLinks.map(sub => (
                                                <Link key={sub._key} href={sub.url || '#'} display="block" px={5} py={2} fontWeight="normal" _hover={{ bg: 'gray.50', color: '#78be20', textDecoration: 'none' }}>
                                                    {sub.label}
                                                </Link>
                                            ))}
                                        </Box>
                                    ...
                                    <Box flex={1} bg="gray.50" p={6}>
                                        {tab.promoBanners && tab.promoBanners.length > 0 ? (
                                            <Box display="grid" gridTemplateColumns={tab.promoBanners.length === 4 ? "repeat(3, 1fr)" : "repeat(2, 1fr)"} gap={4}>
                                                {tab.promoBanners.map((promo, index) => (
                                                    <Box key={promo._key || index} gridColumn={index === 0 ? '1 / -1' : 'span 1'}>
                                                        <Link href={promo.linkUrl || '#'} _hover={{ textDecoration: 'none' }} display="block" h="100%">
                                                            <Box overflow="hidden" borderRadius="md" h="100%" transition="transform 0.2s, box-shadow 0.2s" _hover={{ transform: 'scale(1.02)', shadow: 'md' }}>
                                                                {promo.imageUrl && (
                                                                    <img src={promo.imageUrl.replace('https://cdn.sanity.io', '/mobify/proxy/sanity-images')} alt={promo.altText || 'Promotion'} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                                                                )}
                                                            </Box>
                                                        </Link>
                                                    </Box>
